...
inline asio::awaitable<std::vector<icmp_compose>> async_ping(std::string_view dest, std::size_t count = 3)
{
  using namespace asio::experimental::awaitable_operators;

  auto executor = co_await asio::this_coro::executor;
  asio::ip::icmp::resolver resolver(executor);
  asio::ip::icmp::endpoint destination = *resolver.resolve(icmp::v4(), dest, "").begin();
  asio::ip::icmp::socket socket(executor, asio::ip::icmp::v4());
  std::string body("\"Hello!\" from Asio ping.");
  ...
  std::vector<icmp_compose> composes;
  for (std::size_t sequence_number = 0; sequence_number < count; ++sequence_number) {
          ...
          && icmp_hdr.sequence_number() == sequence_number)
    {
      ...
      composes.emplace_back(std::move(ipv4_hdr), std::move(icmp_hdr), length, std::move(elapsed));
    }
  }

...
public:
  ...
  unsigned char code() const noexcept { return rep_[1]; }
  unsigned short checksum() const noexcept { return decode(2, 3); }
  unsigned short identifier() const noexcept { return decode(4, 5); }
  unsigned short sequence_number() const noexcept { return decode(6, 7); }

  void type(unsigned char n) noexcept { rep_[0] = n; }
  void code(unsigned char n) noexcept { rep_[1] = n; }
  void checksum(unsigned short n) noexcept { encode(2, 3, n); }
